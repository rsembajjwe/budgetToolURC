...
public class BudgetAlerts extends VerticalLayout {
    ...
    private Icon getEnhancedAlertIcon(String type) {
        switch (type) {
            ...
            case "info":
                return new Icon(VaadinIcon.INFO_CIRCLE_O);
            case "success":
    ...
    private List<AlertItem> generateAlerts() {
        ...
        BigDecimal availableBudget = department.getAvailableBudget();

        // Critical alerts
        if (spentPercentage.doubleValue() > 100) {
            alerts.add(new AlertItem(
                    "critical",
                    "high",
                    "🚨 Budget Exceeded - Immediate Action Required",
                    String.format(
                            "Department has exceeded allocated budget by %s (%s%% utilization). "
                            + "This requires immediate attention and corrective measures.",
                            formatCurrency(availableBudget.abs()),
                            spentPercentage.setScale(1, RoundingMode.HALF_UP).toPlainString()
                    ),
        ...
        // Warning alerts
        if (spentPercentage.doubleValue() > 90 && spentPercentage.doubleValue() <= 100) {
            alerts.add(new AlertItem(
                    "warning",
                    "high",
        ...
        if (availableBudget.doubleValue() < 50_000_000 && availableBudget.doubleValue() > 0) {
            alerts.add(new AlertItem(
                    "warning",
                    "medium",
        ...
        // Success alerts
        if (spentPercentage.doubleValue() < 75 && spentPercentage.doubleValue() > 25) {
            alerts.add(new AlertItem(
                    "success",
                    "low",
        ...
        if (spentPercentage.doubleValue() < 40) {
            alerts.add(new AlertItem(
                    "info",
                    "medium",
