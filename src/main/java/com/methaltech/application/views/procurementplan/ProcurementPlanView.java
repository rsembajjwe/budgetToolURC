...
public class ProcurementPlanView extends Div {
    ...
    public ProcurementPlanView(UserService samplePersonService, EmailValidator emailValidator,
        ...
        funds2.setEnabled(false);
        comboBoxD_Section.setEnabled(false);
        comboBoxD_Section2.setEnabled(false);
        ...
        grid.addColumn(new ComponentRenderer<>(urcActivity -> {
            ...
            span = new Span(decimalFormat.format(urcActivity.getCost()));
            return span;
        ...
        budget.addValueChangeListener(e -> {
            if (!e.getValue().isActive()) {
                ...
                budgetItemsGridContextMenu.setEnabled(true);
            }
        ...
        grid.asMultiSelect().addValueChangeListener(event -> {
            ...
            if (event.getValue() != null) {
                setgridBudgetItemsData(event.getValue(), extractCoaSet(event.getValue()));
        ...
        // Configure Grid
        procClassCombo2.setItems(ProcClass.Supplies, ProcClass.Works, ProcClass.Non_Consultancy, ProcClass.Disposal, ProcClass.Other);
        ...
        grid2.addColumn(new ComponentRenderer<>(urcActivity -> {
            ...
            //BigDecimal cost = urcActivity.getCost();
            //BigDecimal cost = Optional.ofNullable(urcActivity.getCost()).orElse(BigDecimal.ZERO);
            // span = new Span(decimalFormat.format(cost != null ? decimalFormat.format(cost) : "0.00"));
            span = new Span(decimalFormat.format(urcActivity.getCost()));
            return span;
    ...
    public Set<COA> extractCoaSet(Set<ProcurementPlan> procurementPlans) {
        return procurementPlans.stream()
                .map(ProcurementPlan::getCoa)
                .filter(coa -> coa != null)
    ...
    private Div createBudgetSelectLayout() {
        ...
        refButton.addClickListener(e -> {
            if (!budget.isEmpty() && budget.getValue() != null) {
                refreshPplan(budget.getValue());
            } else {
                Notification.show("No Period Selected");
            }
        });

        Div div = new Div();
        div.add(budget, procClassCombo, comboBoxD_Section, funds, downloadProcurementReportButton, downloadProcurementReportButtonSheets, refButton);
    ...
    private Div createBudgetSelectLayout2() {
        ...
        budget2.addValueChangeListener(e -> {
            ...
            if (!e.getValue().isActive()) {
                ...
                budgetItemsGridContextMenu2.setEnabled(true);
                contextMenu2.setEnabled(true);
            }
...
// Set margin-right style to create space between components
    ...
    private void createEditorLayout(SplitLayout splitLayout) {
        ...
        Button downloadButton = new Button("Download Excel Report");

        downloadButton.getStyle()
                .set("background-color", "#0b6efd")
                .set("color", "white")
        ...
        downloadButton.addClickListener(event -> {
            List<BudgetItems> allItems = new ArrayList<>();
            gridBudgetItems.getDataProvider().fetch(new Query<>()).forEach(allItems::add);

            if (allItems.isEmpty()) {
                Notification.show("No items to export.", 3000, Notification.Position.MIDDLE);
                return;
            }

            StreamResource resource = new StreamResource(
                    generateFileName(),
                    () -> new BudgetItemExcelExporter().exportBudgetItemsToExcel(allItems)
            );

            downloadAnchor.setHref(resource);

            // Programmatically click the hidden anchor
            downloadAnchor.getElement().executeJs("this.click()");
        });

        //add(downloadButton, gridBudgetItems, downloadAnchor);
        lay.add(downloadButton, gridBudgetItems, downloadAnchor);

        //splitLayout.addToSecondary(gridBudgetItems);
        splitLayout.addToSecondary(lay);
    }

    private String generateFileName() {
        return "Budget_Report_" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss")) + ".xlsx";
    }

    private StreamResource createExcelResource(List<BudgetItems> items) {
        return new StreamResource(generateFileName(), () -> {
            return new BudgetItemExcelExporter().exportBudgetItemsToExcel(items);
        });
    }
    ...
    private void createEditorLayout2(SplitLayout splitLayout) {
        ...
        downloadButton.addClickListener(event -> {
            List<BudgetItems> allItems = new ArrayList<>();
            gridBudgetItems2.getDataProvider().fetch(new Query<>()).forEach(allItems::add);
        ...
        //add(downloadButton, gridBudgetItems, downloadAnchor);
        lay.add(downloadButton, gridBudgetItems2, downloadAnchor);
                        ...
                        if (!procClassCombo2.isEmpty()) {
                            if (procClassCombo2.getValue() == ProcClass.Supplies || procClassCombo2.getValue() == ProcClass.Works || procClassCombo2.getValue() == ProcClass.Non_Consultancy) {
                                ...
                                for (ProcurementPlan pp : p) {
                                    ...
                                    //listBudgetItems.addAll(budgetItemsService.findByBudgetAndProcClassAndCoa(pp.getBudget(), pp.getProcClass(), pp.getCoa()));
                                    listBudgetItems.addAll(pp.getProcPlanBudgetItems());
                    ...
                    if (!procClassCombo2.isEmpty() && !budget2.isEmpty()) {
                        ...
                        for (ProcurementPlan vb : p) {
                            ...
                            for (BudgetItems pp : lists) {
                                ...
                                pr.setProcPlanBudgetItems(Set.of(pp));
                                //pr.setProcPlanBudgetItems(new HashSet<>(listBudgetItems));
                                ...
                                System.out.println(pr.getSubject() + ": " + pr.getCost());
                            ...
                            if (!procClassCombox.isEmpty() && !budget2.isEmpty()) {
                                ...
                                if (!selectedPlans.isEmpty()) {
                                    for (ProcurementPlan p : selectedPlans) {
                                        ...
                                        for (BudgetItems g : p.getProcPlanBudgetItems()) {
                                            g.setProcClass(p.getProcClass());
                                            budgetItemsService.saveBudgetItem(g);
                                        }
    ...
    private class ProcuremetPlanGridContextMenu extends GridContextMenu<ProcurementPlan> {
        ...
        public ProcuremetPlanGridContextMenu(Grid<ProcurementPlan> target) {
            ...
            addItem("Combine Items", e -> e.getItem().ifPresent(person -> {
                List<BudgetItems> listBudgetItems = new ArrayList<>();
                if (target.asMultiSelect().getSelectedItems().size() < 2 || allHaveSameCoa(target.asMultiSelect().getSelectedItems()) == false) {
                    Notificationwarning("You should select more than one Procurement plan Item to Combine Or Selected items are not related");
                } else {
                    if (!target.asMultiSelect().isEmpty()) {
                        if (!procClassCombo.isEmpty() && !budget.isEmpty()) {
                            ...
                            if (procClassCombo.getValue() == ProcClass.Consultancy) {
                                ...
                                procMethod10Box.setItemLabelGenerator(ProcurementMethod::getProcuremntMethod);
                                ComboBox<ProcurementType> contractType2 = new ComboBox("Contract Type");
                                contractType2.setItemLabelGenerator(ProcurementType::getProcuremntType);
                                contractType2.setItems(sampleProcurementTypeService.getAllProcurementTypes());
                                BigDecimalField cost2 = new BigDecimalField("Threshold");
    ...
    private class BudgetItemsGridContextMenu extends GridContextMenu<BudgetItems> {
        ...
        public BudgetItemsGridContextMenu(Grid<BudgetItems> target) {
            ...
            addItem("Transfer to Non Consultancy", e -> e.getItem().ifPresent(person -> {
                ...
                if (target.asMultiSelect().isEmpty() && grid.asMultiSelect().getSelectedItems().size() == 1) {
                    Notificationwarning("You should select a Budget Item to Transfer && A Single Procurement Plan");
                } else {
                    Set<BudgetItems> selectedBudgetItems = target.getSelectedItems();
                    ProcurementPlan oldProcurementPlan = grid.asMultiSelect().getSelectedItems().stream().findFirst().orElseThrow();
                    Set<BudgetItems> oldBudgetItems = oldProcurementPlan.getProcPlanBudgetItems();
                    // Compute difference
                    Set<BudgetItems> remainingBudgetItems = new HashSet<>(oldBudgetItems);
                    remainingBudgetItems.removeAll(selectedBudgetItems);

                    selectedBudgetItems.forEach(item -> {
                        item.setProcClass(ProcClass.Non_Consultancy);
                        budgetItemsService.update(item);
                    });
                    oldProcurementPlan.setProcPlanBudgetItems(remainingBudgetItems);
                    oldProcurementPlan.setCost(generatesumofMonthsFromList(remainingBudgetItems.stream().toList()));
                    oldProcurementPlan.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Non_Consultancy, oldProcurementPlan.getCost()));
                    ProcurementPlanService.save(oldProcurementPlan);

                    BigDecimal tDecimal = generatesumofMonthsFromList(selectedBudgetItems.stream().toList());
                    ProcurementPlan pr = new ProcurementPlan();
                    pr.setSubject(oldProcurementPlan.getCoa().getName());
                    pr.setBudget(oldProcurementPlan.getBudget()); // Assuming all selected plans have the same budget
                    pr.setCoa(oldProcurementPlan.getCoa()); // Assuming all selected plans have the same coa
                    // pr.setProcPlanBudgetItems(new HashSet<>(commonItems));
                    pr.setCurrency(oldProcurementPlan.getCurrency()); // Assuming all selected plans have the same currency
                    ...
                    pr.setProcPlanBudgetItems(selectedBudgetItems);
            ...
            addItem("Transfer to Supplies", e -> e.getItem().ifPresent(person -> {
                ...
                if (target.asMultiSelect().isEmpty() && grid.asMultiSelect().getSelectedItems().size() == 1) {
                    ...
                    selectedBudgetItems.forEach(item -> {
                        item.setProcClass(ProcClass.Supplies);
                        budgetItemsService.update(item);
                    });
                    oldProcurementPlan.setProcPlanBudgetItems(remainingBudgetItems);
                    oldProcurementPlan.setCost(generatesumofMonthsFromList(remainingBudgetItems.stream().toList()));
                    oldProcurementPlan.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Supplies, oldProcurementPlan.getCost()));
                    ProcurementPlanService.save(oldProcurementPlan);
            ...
            addItem("Transfer to Works", e -> e.getItem().ifPresent(person -> {
                ...
                if (target.asMultiSelect().isEmpty() && grid.asMultiSelect().getSelectedItems().size() == 1) {
                    ...
                    selectedBudgetItems.forEach(item -> {
                        item.setProcClass(ProcClass.Works);
                        budgetItemsService.update(item);
                    });
                    oldProcurementPlan.setProcPlanBudgetItems(remainingBudgetItems);
                    oldProcurementPlan.setCost(generatesumofMonthsFromList(remainingBudgetItems.stream().toList()));
                    oldProcurementPlan.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Works, oldProcurementPlan.getCost()));
                    ProcurementPlanService.save(oldProcurementPlan);
            ...
            addItem("Transfer to Other", e -> e.getItem().ifPresent(person -> {
                ...
                if (target.asMultiSelect().isEmpty() && grid.asMultiSelect().getSelectedItems().size() == 1) {
                    ...
                    selectedBudgetItems.forEach(item -> {
                        item.setProcClass(ProcClass.Other);
                        budgetItemsService.update(item);
                    });
                    oldProcurementPlan.setProcPlanBudgetItems(remainingBudgetItems);
                    oldProcurementPlan.setCost(generatesumofMonthsFromList(remainingBudgetItems.stream().toList()));
                    oldProcurementPlan.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Other, oldProcurementPlan.getCost()));
                    ProcurementPlanService.save(oldProcurementPlan);
                    ...
                    pr.setCurrency(oldProcurementPlan.getCurrency()); // Assuming all selected plans have the same currency
                    // pr.setFundsource(plan.getFundsource()); // Assuming all selected plans have the same fund source
                    pr.setProcClass(ProcClass.Other);
                    pr.setProcPlanBudgetItems(selectedBudgetItems);
                    pr.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Other, tDecimal));
                    pr.setCost(tDecimal);
    ...
    private class BudgetItemsGridContextMenu2 extends GridContextMenu<BudgetItems> {
        ...
        public BudgetItemsGridContextMenu2(Grid<BudgetItems> target) {
            ...
            addItem("Transfer to Consultancy", e -> e.getItem().ifPresent(person -> {
                ...
                if (target.asMultiSelect().isEmpty() && grid2.asMultiSelect().getSelectedItems().size() == 1) {
                    Notificationwarning("You should select a Budget Item to Transfer && A Single Procurement Plan");
                } else {
                    Set<BudgetItems> selectedBudgetItems = target.getSelectedItems();
                    ProcurementPlan oldProcurementPlan = grid2.asMultiSelect().getSelectedItems().stream().findFirst().orElseThrow();
                    Set<BudgetItems> oldBudgetItems = oldProcurementPlan.getProcPlanBudgetItems();
                    // Compute difference
                    Set<BudgetItems> remainingBudgetItems = new HashSet<>(oldBudgetItems);
                    remainingBudgetItems.removeAll(selectedBudgetItems);
                    ...
                    selectedBudgetItems.forEach(item -> {
                        item.setProcClass(ProcClass.Consultancy);
                        budgetItemsService.update(item);
                    });
                    oldProcurementPlan.setProcPlanBudgetItems(remainingBudgetItems);
                    oldProcurementPlan.setCost(generatesumofMonthsFromList(remainingBudgetItems.stream().toList()));
                    oldProcurementPlan.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(ProcClass.Consultancy, oldProcurementPlan.getCost()));
                    ProcurementPlanService.save(oldProcurementPlan);
            ...
            add(new Hr());
            addItem("Combine Items 2", e -> e.getItem().ifPresent(person -> {
                List<BudgetItems> listBudgetItems = new ArrayList<>();
                if (target.asMultiSelect().getSelectedItems().size() < 2) {
                    Notificationwarning("You should select more than one Procurement plan Item to Combine");
                } else {
    ...
    private BigDecimal generatesumofProc(List<ProcurementPlan> budgetList) {
        return getTotalCost(budgetList);
    }

    public BigDecimal getTotalCost(List<ProcurementPlan> plans) {
        if (plans == null || plans.isEmpty()) {
            return BigDecimal.ZERO;
        }

        return plans.stream()
                .map(ProcurementPlan::getCost) // extract cost
                .filter(Objects::nonNull) // ignore null costs
    ...
    private void setgridBudgetItemsData(Set<ProcurementPlan> list, Set<COA> coa) {
        ...
        if (budget != null && !budget.isEmpty() && !procClassCombo.isEmpty()) {
            //listBudgetItems.addAll(budgetItemsService.findByBudgetAndProcClassAndSetCoa(budget.getValue(), procClassCombo.getValue(), coa));
            for (ProcurementPlan k : list) {
                listBudgetItems.addAll(k.getProcPlanBudgetItems());
            }
            gridBudgetItems.setItems(listBudgetItems);
            spanTotal.setText(decimalFormat.format(generatesumofMonthsFromList(listBudgetItems)) + "/=");
        }
...
// Create a new style that combines the existing style with the border style
    ...
    @Transactional
    public void refreshProcurementPlan(Budget budget) {
        ProcurementPlanService.deleteProcurementPlansByBudget(budget);

        List<COA> coaList = budgetItemsService.getDistinctCoasByBudget(budget);
        for (COA coa : coaList) {
            ProcurementPlan pr = new ProcurementPlan();
            pr.setSubject(coa.getName());
            pr.setBudget(budget);
            pr.setCoa(coa);
            pr.setProcClass(coa.getProcclass());

            // Sum cost
            BigDecimal cost = budgetItemsService.getTotalCostByBudgetAndCoa(budget, coa);
            pr.setCost(cost != null ? cost : BigDecimal.ZERO);
            Currency findByDataAndBudget = sampleCurrencyService.findCurrenciesByCurrencyShortAndBudget("UGX", budget);

            pr.setCurrency(findByDataAndBudget);

            // Set procurement method
            pr.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(pr.getProcClass(), pr.getCost()));
            if (pr.getCost().doubleValue() > 0) {
                ProcurementPlanService.save(pr);
            }
    ...
    public void refreshPplan(Budget budget) {
        ProcurementPlanService.deleteProcurementPlansByBudget(budget);
        int check = budgetItemsService.syncProcureClassFromCoa();
        System.out.println(check);
        List<CoaProcPlanDTO> results = budgetItemsService.fetchGroupedBudgetItemsByBudget(budget.getId());
        results.forEach(dto -> {
            COA coa = coaService.findById(dto.getCoaId());
            ProcurementPlan pr = new ProcurementPlan();
            pr.setSubject(coa.getName());
            pr.setBudget(budget);
            pr.setCoa(coa);
            pr.setProcClass(coa.getProcclass());
            pr.setCost(dto.getTotalCost());
            Currency findByDataAndBudget = sampleCurrencyService.findCurrenciesByCurrencyShortAndBudget("UGX", budget);

            pr.setCurrency(findByDataAndBudget);

            pr.setProcurementMethod(ProcurementPlanService.getProcurementMethodList2(pr.getProcClass(), pr.getCost()));
            Set<COA> COAHASH = new HashSet<>();
            COAHASH.add(coa);
            List<BudgetItems> bItems = budgetItemsService.findByBudgetAndProcClassAndSetCoa(budget, coa.getProcclass(), COAHASH);
            Set<BudgetItems> coaSet = bItems.stream().collect(Collectors.toSet());

            pr.setProcPlanBudgetItems(coaSet);
            System.out.println(pr.getSubject() + ": " + coaSet.size() + ": Items" + coa.getId());
    ...
    public boolean allHaveSameCoa(Set<ProcurementPlan> procurementPlans) {
        if (procurementPlans == null || procurementPlans.isEmpty()) {
            return false; // or true depending on your business rule
        }

        // Take the first COA as reference
        COA firstCoa = procurementPlans.iterator().next().getCoa();

        // Check if all match the first one
        return procurementPlans.stream()
                .allMatch(plan -> Objects.equals(plan.getCoa(), firstCoa));
    }
