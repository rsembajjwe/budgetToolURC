...
public class BudgetItemsService {
    ...
    GetPeriods perio = new GetPeriods();
    ...
    public BigDecimal calculateTotalByBudgetAndCoaAndSection(Budget budget, COA coacode, UrcDeptSectionAnlDimbgt deptUnits) {
        return repository.calculateTotalByBudgetAndCoaAndSection(budget, coacode, deptUnits);
    }

    public BigDecimal calculateTotalBalanceBudgetAndCoaAndSectionBalance(Budget budget, COA coacode, UrcDeptSectionAnlDimbgt deptUnits, Long requisitionId) {
        BigDecimal budgeted = calculateTotalByBudgetAndCoaAndSection(budget, coacode, deptUnits);
        //BigDecimal actuals = sALFLDGService.calculateTotalByBudgetAndCoaAndSectionActuals(getFinancialYearPeriods(budget), coacode.getCode(), deptUnits.getANL_CODE());
        BigDecimal actuals = requisitionDataRepository.sumRequestedAmountByBudgetCOADeptSection(budget, coacode.getCode(), deptUnits, RequisitionStatus.APPROVED, requisitionId);

        return budgeted.subtract(actuals);
    }

    public BigDecimal calculateTotalBalanceBudgetAndCoaAndActivityAndSectionBalance(Budget budget, COA coacode, Urc_Activities activity, UrcDeptSectionAnlDimbgt deptUnits, Long requisitionId) {
        BigDecimal budgeted = calculateTotalByBudgetAndCoaAndActivityAndSection(budget, coacode, activity, deptUnits);
        //BigDecimal actuals = sALFLDGService.calculateTotalByBudgetAndCoaAndActivityAndSectionActuals(getFinancialYearPeriods(budget), coacode.getCode(),activity.getActivityCode(), deptUnits.getANL_CODE());
        BigDecimal actuals = requisitionDataRepository.sumRequestedAmountByBudgetCOADeptSectionAndActivity(budget, coacode.getCode(), deptUnits, activity.getId(), RequisitionStatus.APPROVED, requisitionId);

        return budgeted.subtract(actuals);
    }

    public BigDecimal calculateTotalActualBudgetAndCoaAndSectionBalance(Budget budget, COA coacode, UrcDeptSectionAnlDimbgt deptUnits) {
        BigDecimal budgeted = calculateTotalByBudgetAndCoaAndSection(budget, coacode, deptUnits);
        BigDecimal actuals = sALFLDGService.calculateTotalByBudgetAndCoaAndSectionActuals(getFinancialYearPeriods(budget), coacode.getCode(), deptUnits.getANL_CODE());
        //BigDecimal actuals=requisitionDataRepository.sumRequestedAmountByBudgetCOADeptSection(budget,coacode.getCode(),deptUnits,RequisitionStatus.APPROVED);

        return budgeted.subtract(actuals);
    }

    public BigDecimal calculateTotalActualBudgetAndCoaAndActivityAndSectionBalance(Budget budget, COA coacode, Urc_Activities activity, UrcDeptSectionAnlDimbgt deptUnits) {
        BigDecimal budgeted = calculateTotalByBudgetAndCoaAndActivityAndSection(budget, coacode, activity, deptUnits);
        BigDecimal actuals = sALFLDGService.calculateTotalByBudgetAndCoaAndActivityAndSectionActuals(getFinancialYearPeriods(budget), coacode.getCode(), activity.getActivityCode(), deptUnits.getANL_CODE());
        //BigDecimal actuals=requisitionDataRepository.sumRequestedAmountByBudgetCOADeptSectionAndActivity(budget,coacode.getCode(),deptUnits,activity.getId(),RequisitionStatus.APPROVED);
    ...
    public BigDecimal calculateTotalIncomeByOrgFundTypeQtr4(Budget budget, FundType fundType) {
        BigDecimal sum = repository.calculateTotalIncomeByOrgFundTypeQtr4(budget, fundType);
        return sum;
    }

    public BigDecimal calculateTotalIncomeByOrgFundTypeQtr3(Budget budget, FundType fundType) {
        BigDecimal sum = repository.calculateTotalIncomeByOrgFundTypeQtr3(budget, fundType);
        return sum;
    }

    public BigDecimal calculateTotalIncomeByOrgFundTypeQtr2(Budget budget, FundType fundType) {
        BigDecimal sum = repository.calculateTotalIncomeByOrgFundTypeQtr2(budget, fundType);
        return sum;
    }

    public BigDecimal calculateTotalIncomeByOrgFundTypeQtr1(Budget budget, FundType fundType) {
        BigDecimal sum = repository.calculateTotalIncomeByOrgFundTypeQtr1(budget, fundType);
        return sum;
    }
    ...
    public List<BudgetItems> findByBudgetAndProcClassAndSetCoa(Budget budget, ProcClass p, Set<COA> coacode) {
        List<BudgetItems> result = repository.findByBudgetAndProcClassAndCoa(budget, p, coacode);
        return result != null ? result : Collections.emptyList();
    }
    ...
    public BigDecimal[] computeGrandExpenditureTotals(Budget budget, Set<UrcDeptSectionAnlDimbgt> deptUnits) {
        List<BudgetItemsActuals> items = findDistinctBudgetItemses22(budget, deptUnits);
        BigDecimal totalSum = BigDecimal.ZERO;
        BigDecimal totalASum = BigDecimal.ZERO;

        // Filter out items with coacode.code starting with "1"
        items = items.stream()
                .filter(item -> item.getCoacode() != null
                && item.getCoacode().getCode() != null
                && !item.getCoacode().getCode().startsWith("1"))
                .toList();
        ...
        for (BudgetItemsActuals item : items) {
            ...
            if (item.getTotalA() != null) {
                totalASum = totalASum.add(item.getTotalA());
            }
        }

        return new BigDecimal[]{totalSum, totalASum};
    }
    ...
    public BigDecimal getTotalExpenditure(Budget budget) {
        return repository.calculateTotalExpenditure(budget);
    }

    public BigDecimal calculateTotalDeptExpenditure(Budget budget, List<UrcDeptSectionAnlDimbgt> deptUnits) {
        return repository.calculateTotalDeptExpenditure(budget, deptUnits);
    }

    public BigDecimal calculateTotalDeptExpenditureQtr1(Budget budget, List<UrcDeptSectionAnlDimbgt> deptUnits) {
        return repository.calculateTotalDeptExpenditureQtr1(budget, deptUnits);
    }

    public BigDecimal calculateTotalDeptExpenditureQtr2(Budget budget, List<UrcDeptSectionAnlDimbgt> deptUnits) {
        return repository.calculateTotalDeptExpenditureQtr2(budget, deptUnits);
    }

    public BigDecimal calculateTotalDeptExpenditureQtr3(Budget budget, List<UrcDeptSectionAnlDimbgt> deptUnits) {
        return repository.calculateTotalDeptExpenditureQtr3(budget, deptUnits);
    }

    public BigDecimal calculateTotalDeptExpenditureQtr4(Budget budget, List<UrcDeptSectionAnlDimbgt> deptUnits) {
        return repository.calculateTotalDeptExpenditureQtr4(budget, deptUnits);
    }

    public BigDecimal calculateTotalDeptExpenditure2(Budget budget, UrcDeptSectionAnlDimbgt deptUnits) {
        return repository.calculateTotalDeptExpenditure(budget, deptUnits);
    }

    public BigDecimal calculateTotalProjectedIncome(Budget budget) {
    ...
    public BigDecimal calculateTotalProjectedIncomeQtr2(Budget budget) {
        return repository.calculateTotalProjectedIncomeQtr2(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQtr3(Budget budget) {
        return repository.calculateTotalProjectedIncomeQtr3(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQtr4(Budget budget) {
        return repository.calculateTotalProjectedIncomeQtr4(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQ1(Budget budget) {
        return repository.calculateTotalProjectedIncomeQ1(budget);
    }

    public BigDecimal calculateTotalOpexBudget(Budget budget) {
        return repository.calculateTotalOpexBudget(budget);
    }

    public BigDecimal calculateTotalCapexBudget(Budget budget) {
        return repository.calculateTotalCapexBudget(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQ2(Budget budget) {
        return repository.calculateTotalProjectedIncomeQ2(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQ3(Budget budget) {
        return repository.calculateTotalProjectedIncomeQ3(budget);
    }

    public BigDecimal calculateTotalProjectedIncomeQ4(Budget budget) {
        return repository.calculateTotalProjectedIncomeQ4(budget);
    }

    public List<COA> getDistinctCoasByBudget(Budget budget) {
        return repository.findDistinctCoacodesByBudget(budget);
    }

    public Set<Fundsource> getDistinctFundsourceByBudgetAndCoa(Budget budget, COA coa) {
        return new HashSet<>(repository.findDistinctFundsourceByBudgetAndCoacode(budget, coa));
    }

    public BigDecimal getTotalCostByBudgetAndCoa(Budget budget, COA coa) {
        return repository.sumOfAllMonthsByBudgetAndCoa(budget, coa);
    }

    public List<CoaProcPlanDTO> fetchGroupedBudgetItemsByBudget(long budget) {
        return repository.fetchGroupedBudgetItemsByBudget(budget);
    }

    public List<BudgetItems> getAllBudgetItems() {
        return repository.findAll();
    }

    public Optional<BudgetItems> getBudgetItemById(Long id) {
        return repository.findById(id);
    }

    public List<BudgetItems> findBudgetItemsExpensesByActvity(Urc_Activities activity) {
        return repository.findBudgetItemsExpensesByActvity(activity);
    }

    public List<BudgetItems> getBudgetItemsByActivityAndCOA(Urc_Activities activity, COA coa) {
        return repository.findByActivityAndCoacodeOrderByItemAsc(activity, coa);
    }

    public List<BudgetItems> getBudgetItemsByCOA(COA coa) {
        return repository.findByCoacodeOrderByItemAsc(coa);
    }

    public List<BudgetItems> getBudgetItemsByBudget(Budget budget) {
        return repository.findByBudgetOrderByItemAsc(budget);
    }

    public List<BudgetItems> getBudgetItemsByCategory(String category) {
        return repository.findByCategoryContainingIgnoreCaseOrderByItemAsc(category);
    }

    public Page<BudgetItems> searchBudgetItems(String searchTerm, Pageable pageable) {
        return repository.searchBudgetItems(searchTerm, pageable);
    }

    @Transactional
    public BudgetItems saveBudgetItem(BudgetItems budgetItem) {
    ...
    public Long getTotalBudgetItemsCount() {
        return repository.count();
    }

    public Long getBudgetItemsCountByActivity(Urc_Activities activity) {
        return repository.countByActivity(activity);
    }

    public Double getTotalCostByActivity(Urc_Activities activity) {
        BigDecimal total = repository.sumTotalCostByActivity(activity);
        return total != null ? total.doubleValue() : 0.0;
    }

    public Double getTotalCostByCOA(COA coa) {
        BigDecimal total = repository.sumTotalCostByCOA(coa);
        return total != null ? total.doubleValue() : 0.0;
    }

    public List<Object[]> getBudgetItemsSummaryByAccountCode(Urc_Activities activity) {
        return repository.getBudgetItemsSummaryByAccountCode(activity);
    }

    public List<Object[]> getBudgetItemsSummaryByCategory(Urc_Activities activity) {
        return repository.getBudgetItemsSummaryByCategory(activity);
    }

    public int syncProcureClassFromCoa() {
        return repository.updateProcureClassFromCoa();
    }
    ...
    public String getDistinctFundSources(Long budgetId, Long activityId) {
        String sources = repository
                .findDistinctFundSourcesByBudgetAndActivity(budgetId, activityId);
        return sources != null ? sources : "Not Specified";
    }

    public String getDistinctFundSources(Long budgetId, Long activityId, Long budgetTypeId) {
        String sources = repository
                .findDistinctFundSourcesByBudgetActivityAndType(budgetId, activityId, budgetTypeId);
        return sources != null ? sources : "Not Specified";
    }

    public String getDistinctFundSources(Long budgetId, Long activityId, Set<Long> budgetTypeIds) {
        String sources = repository
